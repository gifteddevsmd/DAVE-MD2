...
case "readviewonce": {
  try {
    ...
    let type = Object.keys(msg)[0]; // imageMessage, videoMessage, etc.
    if (!type) return m.reply("Cannot detect the message type.");
    ...
    let mediaStream = await downloadContentFromMessage(
      ...
      type.includes("audio") ? "audio" : "buffer"
    );
