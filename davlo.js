...
//━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━//
      ...
      //━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━//
case 'alwaysonline': {
    if (!isBot) return reply(mess.owner); // owner only check
    if (args.length < 1) return reply(`Example ${prefix + command} on/off`);
    ...
    if (q === 'on') {
        ...
        reply(`Invalid option! Example: ${prefix + command} on/off`);
    }
}
break;          
...
case 'darkgpt': {
  if (!text) {
    return m.reply("I'm DarkGPT. I respond to anything, even dark requests. What do you want to ask?");
  }
  try {
    const axios = require('axios');
    ...
    const { data } = await axios.get(`https://api.dreaded.site/api/makgpt?text=${encodedText}`, { timeout: 20000 });

    if (data && data.result) {
      await m.reply(data.result);
    } else {
      await m.reply("No response received. The API might be down or unreachable.");
    }
  } catch (err) {
    console.error("DarkGPT Error:", err);
    await m.reply("❌ Error while contacting the API. It might be down or taking too long. Try again later.");
  }
}
break;
...
case "fullpp": {
  ...
  try {
    ...
    // Download quoted image as buffer
    const stream = await dave.downloadMediaMessage(m.quoted);
    const buffer = Buffer.concat([]);
    for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk]);

    // Update bot profile picture
    await dave.updateProfilePicture(dave.user.id, buffer);
...
case "porn": {
    ...
    // Notice for repentance
    await m.reply("⚠️ Warning: This content is sinful. Consider repenting before continuing.");

    m.reply("Please wait... fetching results");
    ...
    try {
        ...
        await dave.sendMessage(m.chat, { text: resultText }, { quoted: m });
...
case "rent": {
  if (!q) {
    return m.reply(
      "Hello, please provide a valid WhatsApp number.\n\nExample:\n" + prefix + command + " 25411428XXX"
    );
  }
  ...
  try {
    ...
    for (const number of numbers) {
      ...
      const waitMsg = await m.reply(
        "Please wait while the bot generates your pair code for " + number + ".\nThis may take up to 90 seconds on slow servers."
      );
      ...
      try {
        ...
        if (!data || !data.code) {
          return m.reply("Pair code request sent but no code received. Try again in a moment.");
        }

        // Wait a little to simulate processing
        await sleep(3000);

        // Send final pair code
        await dave.sendMessage(m.chat, {
        ...
        if (apiErr.code === "ECONNABORTED") {
          await m.reply("The server took too long to respond. Please try again later.");
        } else {
...
case "obf": {
    ...
    // Get the text from quoted message or current message
    let codeToObfuscate = m.quoted?.text || m.quoted?.message?.conversation || m.text;
    if (!codeToObfuscate) return reply("Quote or send a valid JavaScript code to encrypt!");

    try {
        const obfuscationResult = Obf.obfuscate(codeToObfuscate, {
            compact: true,
        ...
        console.error(err);
        reply("Failed to obfuscate the code. Make sure it's valid JavaScript!");
    }
}
break;
  //━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━//      
...
case 'story':
case 'upsw': {
    // ✅ Strong owner check with fallback
    const senderNum = m.sender.split('@')[0];
    const isOwner = global.owner && global.owner.map(n => n.replace(/[^0-9]/g, '')).includes(senderNum);

    if (!isOwner) {
        return reply('❌ This feature is for *Owner Only!*');
    }
    ...
    try {
        // 🎧 AUDIO STATUS
        if (/audio/.test(mime)) {
            const audioFile = await dave.downloadAndSaveMediaMessage(quoted);
            await dave.sendMessage('status@broadcast', {
                audio: { url: audioFile },
                mimetype: 'audio/mp4',
                ptt: true
            }, { upload: dave.waUploadToServer });
            return reply('✅ *Audio uploaded to status!* 🎧');
        }

        // 🖼️ IMAGE STATUS
        if (/image/.test(mime)) {
            const imageFile = await dave.downloadAndSaveMediaMessage(quoted);
            await dave.sendMessage('status@broadcast', {
                image: { url: imageFile },
                caption: text || '🖼️ Posted via 𝙳𝙰𝚅𝙴-𝙼𝙳'
            }, { upload: dave.waUploadToServer });
            return reply('✅ *Image uploaded to status!* 🖼️');
        }

        // 🎥 VIDEO STATUS
        if (/video/.test(mime)) {
            const videoFile = await dave.downloadAndSaveMediaMessage(quoted);
            await dave.sendMessage('status@broadcast', {
                video: { url: videoFile },
        ...
        // ✍️ TEXT STATUS
        if (!text) return reply('⚠️ Provide text or reply to media!');
        await dave.sendMessage('status@broadcast', { text });
        reply('✅ *Text status uploaded!* ✍️');

    } catch (err) {
        console.error('Status Upload Error:', err);
        reply('❌ Failed to upload status. Try again!');
    }
}
break;
...
case "readviewonce": {
  try {
    if (!m.quoted) return m.reply("⚠️ Reply to a *view-once message* to recover it.");

    // 🛠 Extract quoted message safely
    let msg = m.quoted.message;
    if (!msg) return m.reply("❌ Could not read the quoted message.");

    // ✅ Unwrap ephemeral + viewOnce wrappers
    if (msg?.ephemeralMessage?.message) msg = msg.ephemeralMessage.message;
    if (msg?.viewOnceMessage?.message) msg = msg.viewOnceMessage.message;

    // ✅ Detect message type (imageMessage, videoMessage, audioMessage)
    const type = Object.keys(msg)[0];
    if (!type) return m.reply("❌ Cannot detect message type. Probably not a view-once message.");
    ...
    const mediaStream = await downloadContentFromMessage(
      ...
      type.includes("audio") ? "audio" : "document"
    );
    ...
    if (type.includes("image")) {
      ...
      payload = { audio: buffer, mimetype: msg[type].mimetype || "audio/mpeg", ptt: true };
    } else {
      return m.reply("⚠️ Unsupported view-once message type.");
    }

    // ✅ Send recovered media back to current chat (not DM)
    await dave.sendMessage(m.chat, payload, { quoted: m });

  } catch (error) {
    console.error("RVO ERROR:", error);
    m.reply("❌ Failed to recover view-once message.\nError: " + (error.message || error));
  }
}
break;
